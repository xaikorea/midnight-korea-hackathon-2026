...
- 신규 사례의 별도 동의·관리자 승인 작업과 실행기·독립 검증기 연결은 `docs/program-execution.md`를 참조한다. 실제 거래 성공 여부는 해당 작업의 확정 영수증과 검증 보고서로 확인하며 로컬 테스트와 구분한다.
...
- 운영 기관의 원본 발급·취소 API를 확보하여 합성 발급자 신뢰 배정을 실제 발급자 신뢰로 교체.
