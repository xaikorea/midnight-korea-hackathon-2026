...
웹과 발급기관은 NHN, 증명·노드·인덱서는 별도 Local Devnet이다. 공개 Preprod/메인넷 실행, 실기관 본인확인·공동인증서 검증을 의미하지 않는다. 검증 후 Local Devnet은 중지하고 볼륨·실행 journal을 보존한다. 다시 시연할 때 실행 환경을 시작해야 하며, 기존 작업을 새 계약으로 조용히 교체하지 않는다.
