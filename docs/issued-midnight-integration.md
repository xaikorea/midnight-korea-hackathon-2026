...
이 기록은 `http://127.0.0.1:3120`에서 생성한 합성 작업이다. 공개 NHN 서비스 건을 체인으로 실행한 기록이나 Preprod 증거로 제시하지 않는다. 공개 서버에는 작업 화면·API·관리자 승인이 배포됐지만, 공개 건의 승인과 별도 실행 환경 구성을 마친 뒤 그 환경의 인수시험이 필요하다.
