...
async function call(path:string,body?:unknown){const r=await fetch(path,{cache:'no-store',...(body?{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}:{})});const v=await r.json() as {jobs:Job[];error?:string};if(!r.ok)throw Error(v.error??'처리 상태를 확인하지 못했습니다.');return v;}
export default function ProgramProofPanel({administrator=false,applicationId}:{administrator?:boolean;applicationId?:string}){
 ...
 const [jobs,setJobs]=useState<Job[]>([]),[error,setError]=useState(''),[busy,setBusy]=useState(false),[consent,setConsent]=useState(false);
 async function load(){const v=await call(path);setJobs(v.jobs);}
 useEffect(()=>{let done=false;const update=()=>{if(document.visibilityState==='hidden')return;void call(path).then(v=>{if(!done){setJobs(v.jobs);setError('');}}).catch(e=>{if(!done)setError(e.message);});};update();const timer=setInterval(update,10000);return()=>{done=true;clearInterval(timer);};},[path]);
 ...
 {visible.map(j=><article key={j.id} className="program-chain-job"><h4>{j.name} · {names[j.status]}</h4><p>작업 {j.id}</p>{j.reason&&<p role="status">{j.reason}</p>}<ol className="program-chain-stages">{['신청 동의','관리자 승인','증명·거래','독립 검증'].map((label,i)=>{const n={awaiting_approval:1,queued:2,running:2,awaiting_verification:3,confirmed:4,blocked:0,cancelled:0,needs_attention:2}[j.status];return <li key={label} className={n>i?'complete':n===i&&['running','awaiting_verification'].includes(j.status)?'processing':''}>{label}</li>;})}</ol>
