...
export async function getChatGPTUser(): Promise<ChatGPTUser | null> {
  if(publicDemo()){
    ...
    if(id&&await env.DB!.prepare('SELECT id FROM demo_sessions WHERE id=? AND expires>?').bind(id,Date.now()).first())return {userId:id,storageOwner:id,workspaceKind:'personal',organization:null,displayName:'공개 체험 사용자',email:'demo@example.invalid',fullName:null,allowedRoles:['company',...businessRoles.filter(r=>r!=='company')],authMode:'demo'};
