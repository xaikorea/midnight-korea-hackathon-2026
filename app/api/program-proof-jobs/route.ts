...
export async function GET(req:Request){try{const {actor}=await context(req);if(!publicDemo()||actor.authMode!=='demo')throw new ProgramError('공개 합성 체험에서 이용하세요.',403);return reply({autoRun:demoAutomationEnabled(),jobs:await Promise.all((await listProgramJobs(actor.owner)).filter(j=>j.actor===actor.actor).map(async j=>publicProgramJob(await refreshProgramJob(j))))});}catch(e){return programFailure(e);}}
