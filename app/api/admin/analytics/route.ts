...
export async function GET(req:Request){const user=await serviceAdmin();if(!user)return json({error:'서비스 관리자 로그인이 필요합니다.'},403);
 ...
 await db.prepare('INSERT INTO analytics_admin_audit(id,actor,occurred_at,action,month) VALUES(?,?,?,?,?)').bind(crypto.randomUUID(),user.userId,new Date().toISOString(),'analytics.read',month).run();return json({...report,geo:await geoStatus()});
