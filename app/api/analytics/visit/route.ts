...
import {enrichLocation} from '@/lib/analytics-geo';
export async function POST(req:Request){
 if(req.headers.get('origin')!==requestOrigin(req))return new Response(null,{status:403});
 ...
 const {geoSource,...location}=await enrichLocation(locationInfo(req,env.BIZPROOF_ANALYTICS_CLOUDFLARE==='true',process.env.BIZPROOF_PUBLIC_DEMO==='true'));
 ...
 const res=new NextResponse(null,{status:204,headers:{'Cache-Control':'no-store'}});res.cookies.set('bp-visitor',visitorId,{httpOnly:true,sameSite:'lax',secure:requestOrigin(req).startsWith('https:'),path:'/',maxAge:365*86400});return res;
