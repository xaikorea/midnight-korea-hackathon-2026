...
export default function SimpleApplications({data,refresh,go}:Props){
 ...
 const policies=data.policies.filter(p=>p.status==='active'&&(issuedFlow?['issuer-demo-buyer','issuer-demo-grant'].includes(p.id):(!data.preparedDemo||data.preparedDemo.policyIds.includes(p.id))));
