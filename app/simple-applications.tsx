...
export default function SimpleApplications({data,refresh,go}:Props){
 ...
 async function submit(){
  ...
  try{const r=await fetch('/api/platform',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'submit-applications',companyId:company,consent:true,items})});const value=await r.json() as {error?:string;outcomes:ApplicationBatchOutcome[]};if(!r.ok)throw Error(value.error??'제출에 실패했습니다.');setOutcomes(value.outcomes);setConsent(false);await refresh();}
 ...
 return <div className="simple-workspace batch-workspace">
  <div className="simple-steps"><span className={!selected.length?'active':''}>1 대상 선택</span><ArrowRight size={16}/><span className={pendingIds.length?'active':''}>2 공유 확인·제출</span><ArrowRight size={16}/><span className={existing.length?'active':''}>3 결과</span></div>
