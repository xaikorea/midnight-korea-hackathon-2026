...
export default function EntryForm({demo}:{demo:boolean}){
 ...
 async function start(event:React.FormEvent<HTMLFormElement>){if(!demo){setPending(true);return;}event.preventDefault();setPending(true);setError('');try{const response=await fetch('/signin-with-chatgpt',{method:'POST'});if(!response.ok)throw new Error(response.status===429?'현재 체험 공간이 모두 사용 중입니다. 잠시 후 다시 시도해 주세요.':'체험 공간을 열지 못했습니다. 페이지를 새로고침한 뒤 다시 시도해 주세요.');window.location.assign('/?view=apply');}catch(e){setError(e instanceof Error?e.message:'네트워크 연결을 확인해 주세요.');setPending(false);}}
