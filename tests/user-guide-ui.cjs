...
const publicOnly=process.env.GUIDE_PUBLIC_ONLY==='1';
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#guide-real-programs tbody tr').count(),6);
  assert.equal(await page.locator('#guide-program-midnight tbody tr').count(),8);
  assert.equal(await page.locator('#guide-program-exceptions tbody tr').count(),7);
  const brokenAnchors=await page.locator('.user-guide a[href^="#"]').evaluateAll(links=>links.filter(link=>!document.getElementById(link.hash.slice(1))).map(link=>link.hash));
  assert.deepEqual(brokenAnchors,[]);
  const search=page.getByLabel('이용자 가이드 검색');
  await search.fill('애니메이션');assert.ok(await page.locator('#guide-process').count());assert.ok(await page.locator('article.guide-topic').count()<28);
  ...
  await page.getByRole('button',{name:'모든 안내 보기'}).click();assert.equal(await page.locator('article.guide-topic').count(),28);
  await search.fill('회계연도 불일치');await page.locator('#guide-program-exceptions').waitFor();
  await page.getByRole('link',{name:'새 체인 거래 확인하기 ↓',exact:true}).click();assert.equal(await search.inputValue(),'');
  assert.ok(page.url().endsWith('#guide-program-midnight'));
  assert.match(await page.locator('#guide-program-midnight').innerText(),/이 신규 사례의 체인 실행 승인/);
  assert.match(await page.locator('#guide-official-submission').innerText(),/공식 접수번호를 발급하지 않습니다/);
  ...
  for(const selector of ['.guide-hero','.guide-journeys','#guide-public-demo','#guide-real-programs','#guide-program-midnight','#guide-program-exceptions','#guide-process','#guide-admin-password']){
  ...
  await page.locator('#guide-program-midnight').scrollIntoViewIfNeeded();await page.screenshot({path:'outputs/guide-current-chain-mobile.png'});
  ...
  assert.deepEqual(errors,[]);console.log(JSON.stringify({pass:true,base,publicOnly,topics:28,faq:16,checks:['three journey choices','six case scopes','eight chain states','seven fixtures','anchor integrity','table search and shortcut resets','FAQ','mobile overflow',...(!publicOnly?['signed-in menu and feature links','admin link requires authentication']:[])]}));
