...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('article.guide-topic').count(),21);
  ...
  await search.fill('애니메이션');assert.ok(await page.locator('#guide-process').count());assert.ok(await page.locator('article.guide-topic').count()<21);
  ...
  await page.getByRole('button',{name:'모든 안내 보기'}).click();assert.equal(await page.locator('article.guide-topic').count(),21);
  ...
  assert.deepEqual(errors,[]);console.log(JSON.stringify({pass:true,base,topics:21,faq:12,checks:['current three-step flow','animation and password search','shortcut resets search','FAQ','mobile overflow','anonymous and signed-in guide','menu and feature links','admin link requires authentication']}));
