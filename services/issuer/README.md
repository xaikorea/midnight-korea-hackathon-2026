...
This is an internal versioned protocol, **not** an OpenID4VCI implementation. The same host still shares the kernel and host administrator. The newly issued credential and its original two applications can now be bound to a presenter-approved Midnight job. The public NHN run on 2026-09-25 confirmed eight Local Devnet transactions and a ninth revocation transaction with a separate verifier. See [the issued-job integration](../../docs/issued-midnight-integration.md). The earlier eleven-transaction evidence is a different scenario.
