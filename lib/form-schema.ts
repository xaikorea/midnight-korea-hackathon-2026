...
export type Field={name:string;label:string;type?:string;value?:string;options?:{value:string;label:string}[];help?:string;required?:boolean;allowFuture?:boolean;min?:number;max?:number;showWhen?:{field:string;value:string}};
...
export function formDefinition(fields:Field[]){
 ...
 for(const f of fields){const numeric=f.type==='number';properties[f.name]=numeric?{type:'integer',...(f.min!==undefined?{minimum:f.min}:{}),...(f.max!==undefined?{maximum:f.max}:{})}:{type:'string',...(f.required!==false?{minLength:1,pattern:'\\S'}:{}),...(f.type==='email'?{format:'email'}:{}),...(f.type==='date'?{format:f.allowFuture?'date':'past-date'}:{}),...(f.options?{enum:f.options.map(o=>o.value)}:{})};
