import {automaticDemoOwner,demoApprovalActor} from './demo-automation';
import {z} from 'zod';
...
export async function requestProofJob(owner:string,input:{credentialId:string;requestIds:string[];key:string;consent:boolean}){
 ...
 if(old){const j=await getProofJob(old.id);if(j.credentialId!==input.credentialId||j.requests.map(r=>r.id).sort().join()!==[...input.requestIds].sort().join())fail('다른 요청에 사용된 중복 키입니다.');if(j.status==='awaiting_approval'&&automaticDemoOwner(owner))return approveProofJob(j.id,demoApprovalActor,j.revision);return j;}
 ...
 await current(j);
 if(automaticDemoOwner(owner)){j.status='queued';j.approvedBy=demoApprovalActor;j.approvedAt=new Date().toISOString();j.approvalMode='automatic-synthetic';}
 ...
 const existing=(await allProofJobs(owner)).find(v=>v.credentialId===j.credentialId&&v.requests.map(r=>r.id).sort().join()===j.requests.map(r=>r.id).sort().join()&&!['cancelled','blocked'].includes(v.status));if(existing){if(existing.status==='awaiting_approval'&&automaticDemoOwner(owner))return approveProofJob(existing.id,demoApprovalActor,existing.revision);return existing;}
...
export async function approveProofJob(id:string,actor:string,revision:number){const j=await getProofJob(id);if(j.status!=='awaiting_approval'||j.revision!==revision)fail('승인할 작업 상태를 다시 확인하세요.');await current(j);j.status='queued';j.approvedBy=actor;if(actor===demoApprovalActor)j.approvalMode='automatic-synthetic';j.approvedAt=new Date().toISOString();return save(j);}
