...
export async function evaluateOpa(claims:Claims,policy:Policy,at:Date,options:{address:string;token?:string;fetcher?:typeof fetch}){
 try{const address=opaAddress(options.address);if(options.token&&/[\r\n]/.test(options.token))throw Error();const local=evaluate(claims,policy,at);let deadline:number|null=null;if(policy.maxAgeMonths!==null){const cutoff=new Date(claims.foundedOn);cutoff.setUTCMonth(cutoff.getUTCMonth()+policy.maxAgeMonths);deadline=cutoff.getTime();}
 const facts={revenue:claims.revenue,region:claims.region,certified:claims.certified,now:at.getTime(),deadline};const conditions={minRevenue:policy.minRevenue,maxRevenue:policy.maxRevenue,region:policy.region,requireCertification:policy.requireCertification};const binding=await digest({context:'bizproof:opa:v1',policy,facts});
 ...
 const result=resultSchema.parse(JSON.parse(text).result);const expected={minRevenue:policy.minRevenue===null||claims.revenue>=policy.minRevenue,maxRevenue:policy.maxRevenue===null||claims.revenue<=policy.maxRevenue,age:deadline===null||at.getTime()<=deadline,region:!policy.region||claims.region===policy.region,certification:!policy.requireCertification||claims.certified};
