...
const fileEnvelope=(limit:number)=>z.object({format:z.literal('bizproof-evidence-openbao-v1'),size:z.number().int().positive().max(limit),sha256:z.string().regex(/^[a-f0-9]{64}$/),chunks:z.array(envelopeSchema).min(1).max(Math.ceil(limit/chunkSize))}).strict();
...
export async function protectEvidence(owner:string,key:string,bytes:ArrayBuffer,limit=maxSize){
 if(!Number.isInteger(limit)||limit<1||limit>20*1024*1024)throw Error("Invalid evidence limit");
 if(!bytes.byteLength||bytes.byteLength>limit)throw Error('증빙 크기를 확인하세요.');
...
export async function readEvidence(owner:string,key:string,object:{arrayBuffer:()=>Promise<ArrayBuffer>;customMetadata?:Record<string,string>},expectedHash?:string,limit=maxSize){
 if(!Number.isInteger(limit)||limit<1||limit>20*1024*1024)throw Error("Invalid evidence limit");
 ...
 if(object.customMetadata?.protection==='openbao-v1'){
  ...
  if(bytes.byteLength>Math.ceil(limit*2.5))throw Error('증빙 암호문 크기가 잘못되었습니다.');
  const envelope=fileEnvelope(limit).parse(JSON.parse(new TextDecoder().decode(bytes)));if(envelope.chunks.length!==Math.ceil(envelope.size/chunkSize))throw Error('증빙 조각이 누락되었습니다.');
 ...
 if(bytes.byteLength>limit||expectedHash&&await evidenceHash(bytes)!==expectedHash)throw Error('증빙 무결성 검증 실패');return bytes;
}
