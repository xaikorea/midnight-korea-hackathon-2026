...
export async function verifyRemoteOriginal(raw:unknown,scope:string,requestId:string){
 const c=remoteCredentialSchema.parse(raw),pin=remoteConfig().trust;
 if(c.remoteBinding.scope!==scope||c.remoteBinding.requestId!==requestId||c.keyId!==pin.keyId||c.remoteBinding.documentHash!==await digest(intentBody())||await digest(c.claims)!==await digest(CATALOG.claims)||!await verify(pin.publicKey,credentialPayload(c),c.signature)||Date.parse(c.issuedAt)>Date.now()+5000||Date.parse(c.expiresAt)<=Date.now()||Date.parse(c.expiresAt)<=Date.parse(c.issuedAt))fail('발급기관 서명 또는 신청 연결을 확인하지 못했습니다.',422);
 return c;
}
