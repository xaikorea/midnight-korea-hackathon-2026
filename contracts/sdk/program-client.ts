...
import {pureCircuits,type BusinessClaims,type Request as ChainRequest,type Schnorr_SchnorrSignature} from '../managed-program/contract/index.js';
import {compiledContract,type BusinessContract,type PrivateState} from './program-contract.ts';
import {activateNetwork,address,configuration,type Network,type IndexerConfiguration} from './config.ts';
import {inspectContract,publicProvider,publicReceipt,subscribeContract,zkConfigProvider} from './program-public-state.ts';
...
export class ProgramClient{
 ...
 constructor(options:ClientOptions){
  ...
  this.network=options.network;this.role=options.role;this.privateStateId='bizproof-program:'+options.role;
