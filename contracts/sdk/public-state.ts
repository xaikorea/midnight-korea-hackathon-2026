...
import {address,assetsDirectory,bytes32,circuits,configuration,type Network,type Circuit,type IndexerConfiguration} from './config.ts';
export const zkConfigProvider=new NodeZkConfigProvider<Circuit>(assetsDirectory);
...
export async function inspectContract(provider:PublicDataProvider,network:Network,contractAddress:string,requestHex?:string){
 ...
 const state=await provider.queryContractState(id);if(!state)return {network,contractAddress:id,found:false,checkedAt:new Date().toISOString()};
 ...
 return {network,contractAddress:id,found:true,contractCodeVerified:true,checkedAt:new Date().toISOString(),trustedTime:view.trustedTime.toString(),issuerCount:view.issuerKeys.size().toString(),revokedCount:view.revoked.size().toString(),requestCount:view.requests.size().toString(),resultCount:view.results.size().toString(),...(request?{request}:{}),notice:'인덱서에서 조회한 공개 상태입니다. 업무 자격과의 연결 또는 특정 proof 생성 완료를 자동으로 보장하지 않습니다.'};
}
