...
2026-09-25에 기존 개발 체인을 별도 NHN 서버로 이전하고 실행기·검증기를 상시 프로세스로 구성했다. 기존 9건 거래와 취소 상태를 새 서버에서 다시 대조했다. 사설 `undeployed` 네트워크를 유지하며 새 신청은 원래 공유 동의와 관리자 승인이 필요하다. [전용 서버·복구 검증 기록](docs/nhn-proof-host-2026-09-25.md), [실제 인증 공급자 연결 조건](docs/identity-provider-acceptance.md).
