...
현재 웹 서버와 별도 발급·증명 실행기는 원본 속성을 처리합니다. 기관에는 필요한 조건 결과를 보여주며, 공개 식별값은 연결 가능하고 조건 결과에서 일부 범위를 추론할 수 있습니다. 원본 서류의 사실성은 발급기관의 검토에 의존합니다. 자세한 신뢰 경계는 [아키텍처 문서](docs/privacy-architecture.md)에 정리했습니다.
