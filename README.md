...
신청 화면과 처리 과정 관제의 **블록체인 흐름 보기**에서 기업 자격 → Compact 증명 → 지갑 거래 준비 → 노드 전송 → 블록 확정을 시각화합니다. 구매사·지원사업·미충족·취소 구간을 선택하고 기록을 재생하거나 수동으로 이동할 수 있습니다. 단계별 기술 설명, 공개 범위, 해당 시점까지 확정된 영수증을 확인합니다. 현재 신청의 서버 처리와 별도 Local Devnet 기록을 명시적으로 구분하며, 기록 재생은 거래를 생성하지 않습니다.
